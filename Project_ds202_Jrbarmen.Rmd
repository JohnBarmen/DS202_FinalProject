...
```{r plot2}
ggplot(data2, aes(y = hours, x = married, fill=factor(married)))+geom_boxplot()
ggplot(data2, aes(y = hours, x = marrital, fill=factor(marrital)))+geom_boxplot()
```
...
```{r plot3}
ggplot(data2, aes(y = hours, x = year, fill=factor(year)))+geom_boxplot()
```
...
Train time increases left to right, as the total train time increases the average work week decreases, but not universally. also regardless of trian commute times there is a strong centering on around 35-40 hours a week.  
